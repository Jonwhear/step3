...
export function DailyNote({
  ...
  // signing flips the live prop, and swapping in the signed view at that moment
  // would hide the feedback they just asked for.
  ...
  const locked = readOnly || signedOnArrival;
    ...
    return (
      <Card className="border-good-200 bg-good-50 p-4">
        <p className="text-sm font-medium text-good-700">
          Note signed — rounds complete for hospital day {hospitalDay}
        </p>
    ...
    <div className={`space-y-4 ${pending ? "opacity-70" : ""}`}>
      ...
      {!locked && active.length > 0 ? (
        <form action={signAction}>
          ...
          >
            {signing ? "Signing…" : `Sign note and finish with ${patientName}`}
          </button>
